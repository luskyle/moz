...
| 菜单 | 项（快捷键） |
| --- | --- |
| 文件 | 导出当前帧 STL… (`Ctrl+S`) · 导出当前帧 PNG（引擎渲染）… (`Ctrl+E`) · 保存视图截图… (`Ctrl+Shift+S`) · 导出整个视图序列 PNG…（按当前视角） · 导出动画 GIF… (`Ctrl+G`) · 退出 (`Ctrl+Q`) |
| 视图 | 重置视角 (`Home`) · 视角预设（前视图 / 右视图 / 俯视图 / 等轴测） · 正交投影（勾选切换） · 显示坐标轴 · 显示地面网格（Z=0） · 显示边线（wireframe） · 配色方案（默认 + `color-schemes/render/*.json` 里的全部） |
...
「显示边线」把三角面的**去重唯一边**画成深色线段（立方体 12 个三角面 → 18 条边）。
生成方式是「量化顶点 → `np.unique` 去重」，全向量化，上万面也很快。画实体的那一遍会开
`GL_POLYGON_OFFSET_FILL`，让边线压过共面的面（否则 z-fighting 会闪）。

### 点选测量

**左键单击**（不是拖动）会从屏幕坐标反解一条射线，与（归一化空间的）三角面求最近交点，
状态栏显示：世界坐标、距原点距离、命中的三角面号、以及多部件模式下的部件名。打空显示「未命中」。

- 射线按当前投影构造：透视用 `tan(45°/2)`、正交用 `half = distance*0.55`（与 `_projection` 一致）。
- 命中判定只考虑**可见**部件（隐藏的部件不参与）。

### 多部件视图

`moz.show_parts({"底板": shape_a, "立柱": shape_b})`（或 `(name, Shape)` 序列）：

- 各部件共用一套 center/radius（否则小零件会被放大得和大的一样），合并进**一个**顶点缓冲，
  记录每个部件的顶点区间；绘制时用 `glDrawArrays(mode, first, count)` 只画可见区间（不必重传缓冲）。
- 右侧「部件」列表可勾选显示/隐藏；点选会报告部件名；配色切换按部件逐段重取颜色。
- 「导出当前帧 STL / PNG」导出的是**可见部件的并集**。

### GIF 导出

「导出动画 GIF」逐帧抓 GL 画面并用 **Pillow**（可选依赖）编码 GIF。没装 Pillow 会提示
`pip install Pillow`，不会硬报错——也可以先用「导出整个视图序列 PNG」拿 PNG 序列。
