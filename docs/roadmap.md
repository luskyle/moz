...
- [x] **2D → 3D（图纸变模型）P1**：单视图零件已通——`py/moz_dxf.py` 解析 DXF（ezdxf）→ 图层语义 →
      轮廓修复（去重/隐式闭合/共线合并/自交诊断，只报告不静默；成环与填充规则**与引擎 `import()` 对齐**）
      → 标注取值（与 `dxf_dim` 同列）→ 挤出成型；`py/dxf_demo.py` 走完整闭环（图纸→模型→再出图），
      `py/verify_dxf.py` 拿现成 DXF 语料回归（**75 个文件：OK=61、EMPTY=13、预期坏图 1、FAIL=0**，
      语料含上游 testdata、4 个公开仓库的图纸与真实板框图，见 `corpus/dxf/README.md`），
      并用"原生 `import()` 对拍"验证过（直线段图纸逐位一致）；语料还揪出并修掉了两个健壮性 bug
      （自引用块爆栈、未知实体把整图带崩）。多视图推深度（P4）、B-rep 特征与约束求解
